  ...
  try {
    ...
    switch(type) {
      case 'customers':
        ...
        if (node) {
          relations = await Order.find({ customerId: id });
        }
        break;
      case 'products':
        node = await Product.findById(id);
        if (node) {
          relations = await Order.find({ productIds: id });
        }
        break;
      case 'orders':
      ...
      case 'payments':
        ...
        break;
      default:
        return res.status(400).json({ error: 'Invalid type. Allowed types: customers, products, orders, deliveries, invoices, payments' });
    }
...
// Initialize a large cluster of data for the initial load
router.get('/init', async (req, res) => {
  try {
    const orders = await Order.find().limit(10).lean(); // 10 orders will fan out to ~50 connected nodes
    ...
    for (const order of orders) {
      ...
      if (order.customerId) {
        addNode(order.customerId, 'customers', { _id: order.customerId });
